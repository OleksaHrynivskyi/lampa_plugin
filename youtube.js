(function () {
  ...
  // ─── НАЛАШТУВАННЯ ──────────────────────────────────────────
  // YouTube-контент завжди береться з цього сервера,
  // незалежно від того який основний сервер у Lampa.
  var YOUTUBE_API_HOST = 'https://beta.l-vid.online';
  ...
  function apiUrl(path) {
    ...
    return YOUTUBE_API_HOST + account(normalizedPath);
  }
  ...
  function injectCSS() {
    ...
    style.textContent = [
      ...
      '.ytfeed-host { font-size:0.7em; opacity:0.3; padding:0.3em 1.5em 0; }',
    ].join('\n');
  ...
  function YouTubeFeed(object) {
    ...
    var TABS = [
      ...
      { id: 'subs',   title: 'Підписки',  icon: ICONS.subs },
      { id: 'lists',  title: 'Плейлисти', icon: ICONS.lists },
    ];
    ...
    function doSearch(query) {
      ...
      network.silent(apiUrl(path), function (data) {
        ...
        if (!results.length) {
          showEmpty('Нічого не знайдено за запитом «' + query + '»');
          activateContent();
          return;
        }

        buildGrid('Результати: ' + query, results);
        activateContent();
      }, function () {
        object.activity.loader(false);
    ...
    function loadSubscriptions() {
      ...
      network.silent(apiUrl('/lite/youtube/feed/subscriptions'), function (data) {
        ...
        if (!list.length) {
          showEmpty(data.msg || 'Немає даних. Підключіть YouTube через /youtube_auth у боті.');
          activateContent();
          return;
        }

        buildGrid('Підписки', list);
        activateContent();
      }, function () {
        object.activity.loader(false);
        showEmpty('Помилка завантаження підписок');
        activateContent();
      });
    }
    ...
    function loadPlaylists() {
      ...
      network.silent(apiUrl('/lite/youtube/feed/playlists'), function (data) {
        ...
        if (!pls.length) {
          showEmpty(data.msg || 'Немає плейлистів. Підключіть YouTube через /youtube_auth у боті.');
          activateContent();
          return;
        }
        ...
        object.activity.loader(false);
        showEmpty('Помилка завантаження плейлистів');
        activateContent();
      });
    }
    ...
    function loadPlaylistItems(pl) {
      ...
      network.silent(apiUrl(path), function (data) {
        ...
        if (!results.length) {
          var debugData = typeof data === 'object' ? JSON.stringify(data).substring(0, 80) : 'Невідома відповідь';
          showEmpty('Порожньо. Сервер відповів: ' + debugData + '...');
          activateContent();
          return;
        }
        ...
        object.activity.loader(false);
        showEmpty('Помилка завантаження плейлиста');
        activateContent();
      });
    }
    ...
    function buildGrid(title, gridItems) {
      ...
      gridItems.forEach(function (item) {
        grid.append(makeVideoCard(item));
      });
    ...
    function buildPlaylistGrid(playlists) {
      ...
      playlists.forEach(function (pl) {
        grid.append(makePlaylistCard(pl));
      });
      body.append(grid);
    }
    ...
    function makePlaylistCard(pl) {
      ...
      if (pl.item_count) {
        card.find('.card__view').append(
          '<div class="card__type">' + escapeHtml(String(pl.item_count)) + ' відео</div>'
        );
      }
    ...
    function openVideo(item) {
      ...
      xhr = $.ajax({
        ...
        error: function (jqXHR) {
          ...
          if (jqXHR && jqXHR.status === 0) msg = 'Таймаут — сервер не відповів';
          Lampa.Noty.show(msg);
        }
      });
    }
